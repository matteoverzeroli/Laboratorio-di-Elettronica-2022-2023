...
\begin{itemize}
	\item se $v_{in}<\SI{0.7}{\volt}$ \todo{Verificare il verso, era > ma secondo me è corretto < quindi l'ho cambiato}allora $v\sub{out}=\SI{0}{\volt}$, poiché nel circuito non scorre corrente, nemmeno nella resistenza R che quindi mantiene a massa il nodo $v\sub{out}$;
...
\begin{figure}[!h]
	...
	\end{minipage}
	\caption{Misure del segnale $v_{1}$ (linea gialla) e del segnale $v_{out}$ (linea azzurra). Si è utilizzato in ingresso un segnale sinusoidale con ampiezza picco-picco di \SI{2}{\volt} e frequenza di \SI{1}{\kilo\hertz} (in alto a sinistra), \SI{10}{\kilo\hertz} (in alto a destra), \SI{100}{\kilo\hertz} (in basso a sinistra) e \SI{1}{\mega\hertz} (in basso a destra).}
...
\begin{figure}[!h]
	...
	\end{minipage}
	\caption{Segnale in ingresso (linea gialla) e segnale in uscita (linea azzurra) al circuito. Sono stati utilizzati i cursori forniti dall'oscilloscopio per ottenere le variabili necessarie al calcolo dello \textit{slew rate}.}
	...
	\begin{minipage}{.45\textwidth}
		\scalebox{.658}{
			\begin{circuitikz}
				...
				\draw (0,2) to[R=$R_1$, -*] ++(3,0) ++(.1,-.1) node[below]{$V^-$};
				\draw (3,2) to (4,2);
				\draw (3.7,2) node[op amp, anchor=-](oa){\texttt{TL071}};
				\draw (oa.up) -- ++(0, .3) node[vcc]{$+E$};
				\draw (oa.down) -- ++(0,-.3) node[vee]{$-E$};
				\draw (3,2) -- ++(0,2) coordinate(C) to[R=$R_2$] ++(3.08,0) (C-|oa.out) coordinate(Co) -- (oa.out) to[D=$D_1$] ++(2,0) coordinate(dout) to [short, -o] ++(1,0) node[above]{$v_{out}$};
