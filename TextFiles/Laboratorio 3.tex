...
\end{itemize}
Di conseguenza, questo circuito rettifica entrambe le semionde ed elimina il problema dello \text{shift} di tensione causato dalla tensione di polarizzazione del diodo. Nella figura \ref{fig:uscita_circuito_1} è possibile verificare il comportamento del circuito.
\begin{figure}[h]
	\centering
...
\end{figure}
Si noti come il circuito presenta in uscita il segnale in ingresso rettificato senza alcun shift di tensione. 
...
\begin{equation}
	...
	\end{split}
\end{equation}
\todo{siamo sicuri della prima formula? Io ho un vdd + (vl) e non per}
...
\todo{non mettiamo il calcolo teorico di T1 e T2 da confrontare con quello che abbiamo trovato? Qui mi pare venisse corretto. io poi lo metterei anche nel quarto, anche se non esce giusto pace però è un lavoro visto anche dal punto di vista dei conti che mi pare lo avesse chiesto.}
\def\arraystretch{1.3}
\begin{table}[h]
	\centering
	\begin{tabular}{|c|c|c|}
...
\noindent
Se volessimo variare il \textit{duty-cycle} del segnale in uscita al circuito, si potrebbe sbilanciare l'alimentazione positiva e negativa dell'amplificatore operazionale. Di conseguenza, variano anche le soglie ed è possibile modificare gli intervalli di carica e scarica del condensatore e quindi gli intervalli T\sub{1} e T\sub{2} (\Fig\ref{fig:tensioni_sbilanciare}).
\begin{figure}[h!]
	\centering
