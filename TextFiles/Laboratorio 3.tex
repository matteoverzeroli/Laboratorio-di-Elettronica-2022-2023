...
\end{figure} 
\todo{inseriamo le immagini da 9 a 13?}
\clearpage 
...
\end{figure}
\todo{inserire foto 13}

\todo{inserire le foto successive? non ricordo se sono utili}
...
\begin{equation}
	...
	\end{split}
\end{equation}
Nel circuito realizzato si ottiene quindi che $T_1=T_2$ (considerando $R_1=R_2$ e V\sub{DD}=\SI{10}{\volt} e V\sub{SS}=-\SI{10}{\volt}), ossia che l'onda quadra in uscita presenta un \textit{duty-cycle} pari al \SI{50}{\percent}, dove il \textit{duty-cycle} è definito come $\frac{T_1}{T_1+T_2}$.
...
\todo{inserire immagine confronto tra non sbilanciate e sbilanciate 21 e 22?}

\clearpage
Il quarto circuito realizzato è un circuito oscillatore che permette di regolare il \textit{duty-cycle} dell'onda quadra in uscita senza sbilanciare le tensioni di alimentazione (\Fig\ref{fig:circuito_4}). \todo{inserire foto}
\begin{figure}[h!]
	\centering
	\begin{minipage}{.45\textwidth}
		\scalebox{.75}{
