	...
	\begin{minipage}{.45\textwidth}
		\scalebox{.47}{
			\begin{circuitikz}
				...
				\draw (C.pin 3) to[short, -o] ++(-1,0) ++(0,.1) node[above]{$v_{out}$};
				\draw (C.pin 2) -- ++(-3,0) coordinate(vin);
				\draw (-1,4.02) coordinate (d1) to[potentiometer, name=P] ++(0,2) coordinate(d2);
				\draw [-to, thick](P.wiper) to [out=-90,in=180] node[left, midway] {$R_{T1}$} (d1);
				\draw [-to, thick](P.wiper) to [out=90,in=180] node[left, midway] {$R_{T2}$} (d2);
				%\draw (-1.5, 5.5) node{Pinco};
				\draw (d1) to[D=$D_1$, invert] ++(2,0) -- ++(0,1) coordinate(dend);
				\draw (d2) to[D=$D_2$] ++(2,0) -- (dend);
				\draw (vin) -- (crgnd) -- ++(0,2.5) coordinate(tb) to[short, -*] (P.wiper);
				\draw (dend) to[short, *-] (tb-|vccj) -- (vccj) to[short, -*] (C.pin 7 -| vccj);
				\draw (vin) to[C=$C$] ++(0,-4.82) node[ground]{};
				\draw[thick] (-8,-5.3) rectangle (6.6,6.9);
			\end{circuitikz}
		}
	\end{minipage}\qquad
	\begin{minipage}{.45\textwidth}
